  ...
  return (
    <main className={styles.main}>
      ...
      <section className={styles.section}>
        ...
        <div className={styles.products}>{featuredProducts.map((product) => <article className={styles.product} key={product.id}><Link href={`/produtos/${product.id}`} className={styles.productImage}><Image src={product.image} alt={product.name} fill sizes="(max-width: 600px) 50vw, (max-width: 900px) 50vw, 25vw" className={styles.productPhoto} /><span>NOVO</span></Link><div className={styles.productInfo}><small>{product.category}</small><Link href={`/produtos/${product.id}`}><h3>{product.name}</h3></Link><strong>R$ {product.price.toFixed(2).replace(".", ",")}</strong><Link href={`/produtos/${product.id}`} className={styles.productButton}>Ver produto</Link></div></article>)}</div>
      </section>
