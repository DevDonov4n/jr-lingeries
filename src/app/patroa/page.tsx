...
export default async function PatroaPage() {
  ...
  const [products, categories, clients, paid, pending, pendingSuggestions] = await Promise.all([
  ...
  return <PatroaClient products={products.map((p) => ({ id: p.id.toString(), name: p.name, categoryId: p.category_id?.toString() ?? "", stock: p.stock_quantity }))} categories={categories.map((c) => ({ id: c.id.toString(), name: c.name }))} clients={clients} received={Number(paid._sum.total ?? 0)} toReceive={Number(pending._sum.total ?? 0)} pendingSuggestions={pendingSuggestions} />;
}
