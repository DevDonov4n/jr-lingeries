...
export default function ImportarPedidoPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(
        `Pedido ${result.pedido} encontrado: ${result.total} produto(s).`,
      );
      for (const product of result.products ?? [])
        analyzeProduct(product.sku, product.imageUrl);
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : "Erro ao consultar o pedido.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function analyzeProduct(sku: string, imageUrl: string) {
    setAnalyzing(sku);
    try {
      const response = await fetch("/api/import-pedido/analisar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const result = await response.json();
      if (!response.ok)
        throw new Error(result.error || "Não foi possível analisar o produto.");
      setProducts((current) =>
        current.map((product) =>
          product.sku === sku
            ? {
  ...
  async function actionProduct(product: ImportedProduct) {
    ...
    try {
      const category = categories.find(
        (item) => item.name === product.analysis?.category,
      );
      const body =
        product.status === "EXISTENTE"
          ? { action: "estoque", sku: product.sku, quantity: product.quantity }
          : {
              action: "cadastrar",
              sku: product.sku,
              name: product.analysis?.name || `Produto ${product.sku}`,
              ...
              costPrice: product.analysis?.costPrice ?? 0,
              salePrice: product.analysis?.salePrice ?? 0,
          ...
          <section className={styles.results}>
            ...
            <div className={styles.productList}>
              {products.map((product) => {
                ...
                const newStock =
                  (product.existing?.stockQuantity ?? 0) + product.quantity;
                return (
                  <article
                    className={`${styles.productCard} ${existing ? styles.existing : styles.newProduct}`}
                    key={product.sku}
                  >
                    ...
                    <div className={styles.productInfo}>
                      ...
                      {existing ? (
                        <>
                          <h3>{product.existing?.name}</h3>
                          <p className={styles.meta}>
                            {product.existing?.category ?? "Sem categoria"} ·
                            Estoque atual:{" "}
                            {product.existing?.stockQuantity ?? 0}
                          </p>
                          <div className={styles.stockBox}>
                            <span>Quantidade no pedido</span>
                            <strong>+{product.quantity}</strong>
                            <small>Novo estoque: {newStock}</small>
                          </div>
                          <button
                            ...
                            onClick={() => actionProduct(product)}
                          >
                            {product.actionDone
                              ? "Estoque atualizado ✓"
                              : acting === product.sku
                                ? "Atualizando..."
                                : `Adicionar ${product.quantity} ao estoque`}
                          </button>
                        </>
                      ) : (
                        <>
