...
type FormColor = { color: string; colorHex: string; stock: string };
type FormState = {
  name: string;
  categoryId: string;
  description: string;
...
const emptyForm = (categoryId = ""): FormState => ({
  ...
  minimumStock: "0",
  imageUrl: "",
  colors: [{ color: "", colorHex: "#d2a58a", stock: "0" }],
});
...
export default function EstoqueClient({
  ...
  function openEdit(p: Product) {
    ...
    setForm({
      ...
      colors: p.colors.length
        ...
        : [{ color: p.color, colorHex: "#d2a58a", stock: String(p.stock) }],
    });
  ...
  function selectImage(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.type.startsWith("image/"))
      return setMessage("Selecione um arquivo de imagem.");
    if (file.size > 5 * 1024 * 1024)
      return setMessage("A imagem deve ter no máximo 5 MB.");
  ...
  async function uploadImage(file: File) {
    ...
    const response = await fetch("/api/patroa/cloudinary/upload", {
      method: "POST",
      body: data,
    });
  ...
  function updateColor(index: number, field: keyof FormColor, value: string) {
    setForm((current) => ({
      ...current,
  ...
  const totalColorStock = form.colors.reduce(
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      if (form.colors.some((item) => !item.color.trim()))
        throw new Error(
          "Informe o nome de todas as cores ou remova as linhas vazias.",
        );
      ...
      if (file) {
        setMessage("Enviando imagem para o Cloudinary...");
      ...
      data.set("stock_quantity", String(totalColorStock));
      if (editingId) {
  ...
  async function remove(p: Product) {
    if (!window.confirm(`Deseja realmente excluir “${p.name}”?`)) return;
    setSaving(true);
    setMessage("");
    try {
      const data = new FormData();
  ...
  async function toggle(p: Product) {
    ...
    try {
      const data = new FormData();
      data.set("id", p.id);
      await toggleProductStatus(data);
      setProducts((list) =>
        list.map((item) =>
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.summary}>
        ...
        <span>·</span>
        <span>
          {filtered.reduce((sum, p) => sum + p.stock, 0)} peças em estoque
        </span>
      </section>
          ...
          filtered.map((p) => (
            <article className={styles.product} key={p.id}>
              ...
              <div className={styles.info}>
                ...
                <span>
                  {p.category} · SKU: {p.sku || "não informado"}
                </span>
                <span>
                  {p.stock} peças · {currency(p.price)}
                </span>
                {p.colors.length > 0 && (
                  <small>
                    {p.colors.length} cor(es):{" "}
                    {p.colors.map((color) => (
                      <span key={color.color} className={styles.colorSummary}>
                        <i style={{ backgroundColor: color.colorHex }} />
                        {color.color} ({color.stock})
                      </span>
                    ))}
                  </small>
                )}
                <small className={p.active ? styles.active : styles.inactive}>
                  {p.active ? "Ativo" : "Inativo"}
                  {p.stock <= p.minimumStock ? " · Estoque baixo" : ""}
                </small>
              </div>
              <div className={styles.actions}>
                <button onClick={() => openEdit(p)} disabled={saving}>
                  Editar
                </button>
                <button onClick={() => void toggle(p)} disabled={saving}>
        ...
        >
          <div className={styles.modal}>
            ...
            <form onSubmit={submit} className={styles.form}>
              ...
              />
              <section className={styles.colorManager}>
                <div className={styles.colorManagerHeader}>
                  <div>
                    <strong>Cores e estoque</strong>
                    <span>
                      Escolha a cor visual de cada variação e controle seu
                      estoque.
                    </span>
                  </div>
                ...
                {form.colors.map((item, index) => (
                  <div
                    className={styles.colorRow}
                    key={`${index}-${item.color}`}
                  >
                    <div className={styles.colorNameField}>
                      <input
                        value={item.color}
                        onChange={(e) =>
                    ...
                    <label className={styles.colorPickerLabel}>
                      ...
                      <input
                        ...
                        aria-label={`Escolher cor visual de ${item.color || `variação ${index + 1}`}`}
                      />
                    </label>
                    <input
                      type="number"
                ...
                <small>
                  Estoque total: <strong>{totalColorStock}</strong> peça(s)
                </small>
              </section>
              <textarea
                name="description"
              ...
              <input
                ...
                placeholder="Estoque mínimo geral"
                required
              />
