...
export default async function EstoquePage() {
  ...
  for (const variant of variants) {
    ...
    list.push({ color: variant.color, stock: Number(variant.stock_quantity), active: Boolean(variant.active) });
  ...
  return (
    <EstoqueClient
      initialProducts={products.map((p) => ({
        ...
        colors: variantMap.get(p.id.toString()) ?? (p.color ? [{ color: p.color, stock: p.stock_quantity, active: p.stock_quantity > 0 }] : []),
      }))}
