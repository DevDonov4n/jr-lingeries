...
export default async function EstoquePage() {
  ...
  const variants = products.length
    ? await prisma.$queryRaw<Array<{ product_id: bigint; color: string; color_hex: string; stock_quantity: number; active: boolean }>>`SELECT product_id, color, color_hex, stock_quantity, active FROM product_variants WHERE product_id IN (${Prisma.join(products.map((product) => product.id))}) ORDER BY color ASC`
    : [];
  const variantMap = new Map<string, { color: string; colorHex: string; stock: number; active: boolean }[]>();
  for (const variant of variants) {
    const key = variant.product_id.toString();
    const list = variantMap.get(key) ?? [];
    list.push({ color: variant.color, colorHex: variant.color_hex, stock: Number(variant.stock_quantity), active: Boolean(variant.active) });
  ...
  return (
    <EstoqueClient
      initialProducts={products.map((p) => ({
        ...
        colors: variantMap.get(p.id.toString()) ?? (p.color ? [{ color: p.color, colorHex: "#d2a58a", stock: p.stock_quantity, active: p.stock_quantity > 0 }] : []),
      }))}
