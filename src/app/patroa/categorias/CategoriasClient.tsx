...
export default function CategoriasClient({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (modal === "create") await createCategory(data);
      else await updateCategory(data);
      setModal(null);
      setMessage(
        editing
  ...
  async function toggle(category: Category) {
    try {
      ...
      await toggleCategoryStatus(data);
  ...
  async function remove(category: Category) {
    if (category.productCount > 0) {
      setMessage(
        `A categoria "${category.name}" possui ${category.productCount} produto(s). Remova ou mova os produtos antes de excluí-la.`,
      );
      return;
    }
    ...
    <main className={styles.page}>
      ...
      <header className={styles.header}>
        <div>
          ...
          <h1>Categorias</h1>
          <p>Organize os produtos da loja por categorias.</p>
        </div>
          ...
          categories.map((category) => (
            <article className={styles.category} key={category.id}>
              <div className={styles.categoryInfo}>
                <div className={styles.categoryIcon}>C</div>
                <div>
                  <h2>{category.name}</h2>
              ...
              <div className={styles.categoryCount}>
                ...
                <span>{category.productCount === 1 ? "item" : "itens"}</span>
              </div>
              <span
                className={category.active ? styles.active : styles.inactive}
              >
                {category.active ? "Ativa" : "Inativa"}
              </span>
              <div className={styles.actions}>
                <button onClick={() => openEdit(category)}>Editar</button>
