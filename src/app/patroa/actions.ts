...
function normalizeHex(value: unknown) {
  ...
  if (!/^#[0-9a-fA-F]{6}$/.test(hex))
    throw new Error("A cor visual deve estar em formato hexadecimal válido.");
  return hex.toLowerCase();
}
function parseColors(formData: FormData) {
  const raw = text(formData, "colors");
  if (!raw) return [] as { color: string; colorHex: string; stock: number }[];
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch {
    throw new Error("Formato das cores inválido.");
  }
  ...
  const colors = parsed.map((item) => {
    ...
    const colorHex = normalizeHex((item as { colorHex?: unknown }).colorHex);
    const stock = Number((item as { stock?: unknown }).stock ?? 0);
    if (!color || color.length > 50)
      throw new Error("Cada cor deve ter entre 1 e 50 caracteres.");
    if (!Number.isInteger(stock) || stock < 0)
      throw new Error(`Estoque inválido para a cor ${color}.`);
    return { color, colorHex, stock };
  });
  const normalized = colors.map((item) =>
    item.color.toLocaleLowerCase("pt-BR"),
  );
  if (new Set(normalized).size !== normalized.length)
    throw new Error("Não é possível cadastrar a mesma cor duas vezes.");
  return colors;
}
async function replaceVariants(
  productId: bigint,
  colors: { color: string; colorHex: string; stock: number }[],
) {
  await prisma.$executeRaw`DELETE FROM product_variants WHERE product_id = ${productId}`;
  for (const variant of colors)
    await prisma.$executeRaw`INSERT INTO product_variants (product_id, color, color_hex, stock_quantity, active) VALUES (${productId}, ${variant.color}, ${variant.colorHex}, ${variant.stock}, ${variant.stock > 0})`;
}
...
export async function createCategory(formData: FormData) {
  ...
  if (await prisma.categories.findUnique({ where: { name } }))
...
export async function updateCategory(formData: FormData) {
  ...
  if (name.length > 100)
    throw new Error("O nome da categoria deve ter no máximo 100 caracteres.");
  if (!(await prisma.categories.findUnique({ where: { id } })))
...
export async function deleteCategory(formData: FormData) {
  ...
  if (category._count.products > 0)
    throw new Error(
      "Esta categoria possui produtos vinculados. Mova os produtos para outra categoria antes de excluí-la.",
    );
...
export async function createProduct(formData: FormData) {
  ...
  const legacyStock = nonNegativeInt(formData, "stock_quantity");
  const stock = colors.length
    ? colors.reduce((sum, item) => sum + item.stock, 0)
    : legacyStock;
  const categoryId = optionalBigInt(formData, "category_id");
  const legacyColor = colors[0]?.color ?? (text(formData, "color") || null);
  ...
  if (stock > 0)
    await prisma.inventory_movements.create({
      data: {
        ...
        previous_stock: 0,
        current_stock: stock,
        reason: colors.length
          ? "Estoque inicial das variações de cor"
...
export async function updateProduct(formData: FormData) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const variant of colors)
      await tx.$executeRaw`INSERT INTO product_variants (product_id, color, color_hex, stock_quantity, active) VALUES (${id}, ${variant.color}, ${variant.colorHex}, ${variant.stock}, ${variant.stock > 0})`;
    if (stock !== current.stock_quantity)
      await tx.inventory_movements.create({
        data: {
          ...
          current_stock: stock,
          reason: colors.length
            ? "Ajuste de estoque pelas variações de cor"
            : "Ajuste de estoque pelo dashboard da patroa",
        },
      });
  });
...
export async function deleteProduct(formData: FormData) {
  ...
  if (product.sale_items.length > 0 || product.offer_items.length > 0)
    throw new Error(
      "Este produto já possui vendas ou ofertas vinculadas. Desative o produto em vez de excluí-lo.",
    );
