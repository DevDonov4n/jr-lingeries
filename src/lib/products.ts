...
async function getVariants(productIds?: bigint[]) {
  if (productIds && productIds.length === 0)
    return new Map<string, ProductColor[]>();
  const rows = productIds
    ? await prisma.$queryRaw<
        VariantRow[]
      >`SELECT product_id, color, color_hex, stock_quantity, active FROM product_variants WHERE product_id IN (${Prisma.join(productIds)}) ORDER BY color ASC`
    : await prisma.$queryRaw<
        VariantRow[]
      >`SELECT product_id, color, color_hex, stock_quantity, active FROM product_variants ORDER BY color ASC`;
...
function mapProduct(
  ...
  const activeOffers = (product.offer_items ?? [])
    ...
    .filter(
      (offer) => offer.active && offer.starts_at <= now && offer.ends_at >= now,
    );
...
export async function getProducts(): Promise<Product[]> {
  ...
  return products.map((product) =>
    mapProduct(product, variants.get(product.id.toString()) ?? []),
  );
}
