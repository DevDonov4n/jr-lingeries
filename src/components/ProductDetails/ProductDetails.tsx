...
function colorValue(color: string, colorHex?: string) {
  if (colorHex && /^#[0-9a-f]{6}$/i.test(colorHex)) return colorHex;
  const normalized = color.toLocaleLowerCase("pt-BR").trim();
  return (
    colorMap[normalized] ??
    (/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(normalized) ? normalized : "#d2a58a")
  );
}
...
export default function ProductDetails({ product }: ProductDetailsProps) {
  ...
  const availableColors = product.colors.filter(
    (color) => color.active && color.stock > 0,
  );
  const [selectedSize, setSelectedSize] = useState(product.sizes[0]);
  const [selectedColor, setSelectedColor] = useState(
    availableColors[0]?.color ?? product.color ?? "",
  );
    ...
    <>
      {product.colors.length > 0 && (
        <div className={styles.option}>
          <h2>
            Cor: <span>{selectedColor || "Selecione"}</span>
          </h2>
          <div className={styles.colors}>
            {product.colors.map((variant) => {
              const available = variant.active && variant.stock > 0;
              return (
                <button
                  key={variant.color}
                  type="button"
                  title={
                    available ? variant.color : `${variant.color} - Esgotada`
                  }
                  aria-label={
                    available ? variant.color : `${variant.color} - Esgotada`
                  }
                  ...
                  className={`${styles.colorButton} ${selectedColor === variant.color ? styles.selectedColor : ""} ${!available ? styles.unavailableColor : ""}`}
                >
                  <span
      ...
      >
        {selectedStock > 0 ? "Adicionar ao carrinho" : "Produto esgotado"}
      </button>
    </>
  );
}
